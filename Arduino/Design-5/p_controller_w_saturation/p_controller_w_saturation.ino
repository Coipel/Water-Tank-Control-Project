...
void loop() {
  ...
  for (int i = 0; i < data_size; i++) {
    ...
    // ---Setpoint Process and Data Sample---
    setpoint[i] = map(analogRead(POTENTIOMETER_PIN), 0, 1023, 0, 40);
    float error = setpoint[i] - strain[i];
    
    // Proportional only controller
    float P = 226.779;  // Value determined by system identification and pid tuner on MATLAB
    float accuation = P*error;
    ...
    valve[i] = round(accuation);
    
    // ---Turn on pump motor after data sample---
    analogWrite(PWMA_PIN, map(valve[i], 0, 137, 118, 255));
    delay(powered_delay);
